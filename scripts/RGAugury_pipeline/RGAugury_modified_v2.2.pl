...
#modified by Jiangqian at 2022/03/15, 2022/09/14
#modified: 
#1.identification of RPW8; 
#2.count all each signle or combined domain architectures respectively, which results in a "0" value in original "other" column;
#3.filter pfam_out by Evalue($e_dom_filter=0.001), which saves time to re-run pfam by a new $e_dom.
use strict;
use Getopt::Long;
...
system("python /data01/jiangqian/RGAugury/RGAugury_pipeline/pfam_out_filter.py -i $pfam_out -e $e_dom_filter -o $pfam_out_filter");
