...
#Author: Jiang Qian
#Data: 2021.11.25
#extract all/special domains in domain pos file with -n
...
parser.add_argument('-m', '--match', required=True, help="match type (e.g., RNL/CNL/TNL)")
parser.add_argument('-n', '--name', required=True, help="domain name (e.g., all/NBS/NBS,LRR)")
...
matchType = (args.match).split(',')
key = args.key
domainPos = args.domain
domain_name = (args.name).split(',')
seqfile = os.path.dirname(os.path.abspath(domainPos)) + os.sep + key + '.formated.protein.input.fas'
output1 = args.output + os.sep + args.key + "_domain_name." + "_".join(matchType)
output2 = args.output + os.sep + args.key + "_domain_seq." + "_".join(matchType)
...
for line in PosFile:
    ...
    for domain in domains:
        ...
        name = string.split('domain_')[1] #NBS/LRR
        get = "yes"
        if 'all' not in domain_name:
